class Character {
    constructor(scene, config) {
        ...
        // Phase 3: `id` is the backend poet id (saadi, hafez, ...) while
        // `spriteId` points at the reused character atlas (socrates, plato,
        // ...). All texture frame names are derived from spriteId.
        this.spriteId = config.spriteId || config.id;
        this.spawnPoint = config.spawnPoint;
        this.atlas = config.atlas;
        this.defaultFrame = `${this.spriteId}-${config.defaultDirection || 'front'}`;
    ...
    createAnimations() {
        ...
        directions.forEach(direction => {
            const animKey = `${this.spriteId}-${direction}-walk`;

            if (!anims.exists(animKey)) {
                anims.create({
                    key: animKey,
                    frames: anims.generateFrameNames(this.atlas, {
                        prefix: `${this.spriteId}-${direction}-walk-`,
                        end: 8,
                        zeroPad: 4,
                    }),
                    frameRate: 10,
    ...
    facePlayer(player) {
        ...
        if (Math.abs(dx) > Math.abs(dy)) {
            this.sprite.setTexture(this.atlas, `${this.spriteId}-${dx < 0 ? 'left' : 'right'}`);
        } else {
            this.sprite.setTexture(this.atlas, `${this.spriteId}-${dy < 0 ? 'back' : 'front'}`);
        }
    }
    ...
    chooseNewDirection() {
        ...
        if (Math.random() < 0.4) {
            ...
            const animKey = `${this.spriteId}-${this.getDirectionFromMovement()}-walk`;
            if (this.scene.anims.exists(animKey)) {
                this.sprite.anims.play(animKey);
            } else {
                this.sprite.setTexture(this.atlas, `${this.spriteId}-${this.getDirectionFromMovement()}`);
            }
            ...
            this.sprite.setTexture(this.atlas, `${this.spriteId}-${direction}`);
